...
class game_factory {
    public:
        ...
        static enemy create_enemy_green(Vector2 position) {
            return enemy(position, {64,64}, true, 5, {-200,0}, 0, SLIMEGREEN);
        }
        static enemy create_enemy_black(Vector2 position) {
            return enemy(position, {96,96}, true, 30, {-100,0}, 0, SLIMEBLACK);
        }
        static enemy create_enemy_red(Vector2 position) {
            return enemy(position, {32,32}, true, 3, {-400,0}, 0, SLIMERED);
        }
        static enemy create_enemy_purple(Vector2 position) {
            return enemy(position, {72,72}, true, 10, {-180,0}, 0, SLIMEPURPLE);
        }
        static enemy create_enemy_blue(Vector2 position) {
            return enemy(position, {72,72}, true, 10, {-180,0}, 0, SLIMEBLUE);
        }
        static flying_enemy create_enemy_angel(Vector2 position) {
            return flying_enemy(position, {64,64}, true, 10, {-180,0}, 0, FLYINGANGEL, 80, 2.5);
        }
        static flying_enemy create_enemy_bird(Vector2 position) {
            return flying_enemy(position, {32,32}, true, 5, {-200,0}, 0, FLYINGBIRD, 120, 3.0);
        }
        static flying_enemy create_enemy_dragon(Vector2 position) {
            return flying_enemy(position, {128,128}, true, 50, {-150,0}, 0, FLYINGDRAGON, 150, 2.0);
        }
        // castle(位置, 大小, 活著, 血量)
