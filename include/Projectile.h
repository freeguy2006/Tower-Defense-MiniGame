...
class projectile : public game_object{
    private:
        ...
        float _splash_damage = 0;           // 範圍傷害
        float _poison_damage = 0;           // 毒傷害
        float _poison_interval = 3.0f;      // 毒間隔
        float _crit_chance = 0;             // 暴擊機率
        float _crit_damage = 0;             // 暴擊傷害
        float _crit_multiplier = 1.0f;      // 暴擊倍率
        float _crit_hp_percent = 0;         // HP百分比暴擊
        float _cooldown_multiplier = 1.0f;  // 攻速倍率

    public:
        projectile(Vector2 position, Vector2 size, bool active, float damage, Vector2 speed): game_object(position, size, active), _damage(damage), _speed(speed){}
        ~projectile() = default;
        ...
        // setter
        void set_damage(float damage) { _damage = damage; }
        ...
        void set_splash_range(float splash_range) { _splash_range = splash_range; }
        void set_splash_damage(float splash_damage) { _splash_damage = splash_damage; }
        void set_poison_damage(float poison_damage) { _poison_damage = poison_damage; }
        ...
        void set_crit_damage(float crit_damage) { _crit_damage = crit_damage; }
        ...
        float get_splash_damage() const { return _splash_damage; }
        float get_poison_damage() const { return _poison_damage; }
        float get_poison_interval() const { return _poison_interval; }
        float get_crit_chance() const { return _crit_chance; }
        float get_crit_damage() const { return _crit_damage; }
